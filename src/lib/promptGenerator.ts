...
export function generateIdeaPrompt(project: Project): string {
  ...
  return `あなたはLINEスタンプの企画・キャラクターデザインを支援するクリエイターです。以下の情報から、${typeLabel}の企画と${count}個のスタンプ案を作成してください。\n\n【プロジェクト名】${project.name}\n【スタンプ種類】${typeLabel}\n【アイデア】${idea.rawIdea}\n【ターゲット】${idea.target}\n【利用シーン】${idea.usageScene}\n【雰囲気】${idea.mood}\n【その他の希望】${idea.notes}\n\n重要：回答は説明文やMarkdownを付けず、次のJSON形式だけで返してください。stampsは必ず${count}件にしてください。commentは実際にスタンプ画像へ入れる短く自然な日本語にしてください。emotion、expression、pose、sceneはcommentに合う内容にしてください。${isAnimated(project) ? 'アニメーションスタンプなので、startPose、motion、endPose、loop、frameCountも必ず入れてください。frameCountは5〜20の範囲にしてください。' : ''}\n\n{\n  "concept": "企画コンセプト",\n  "characterDirection": "キャラクター方向性",\n  "designStyle": "デザインテイスト",\n  "usageScene": "主な利用シーン",\n  "dialogueDirection": "セリフ全体の方向性",\n  "differentiation": "差別化ポイント",\n  "stamps": [\n    { "number": 1, "comment": "ありがとう！", "emotion": "嬉しい", "expression": "笑顔", "pose": "お辞儀", "scene": "感謝を伝える"${isAnimated(project) ? ', "startPose": "正面で軽く立つ", "motion": "ゆっくりお辞儀する", "endPose": "笑顔で戻る", "loop": "自然に最初へ戻る", "frameCount": "8"' : ''} }\n  ]\n}\n\n${count}件は日常で実際に使う頻度と企画コンセプトの両方を考えて選定してください。LINE公式サービスと誤認される表現や既存キャラクターの模倣は避け、オリジナル作品として提案してください。`;
}
...
export function generateAnimatedStickerPrompt(project: Project): string {
  const { character, concept } = project;
  const stampLines = project.stamps.map((s) => `APNG ${String(s.number).padStart(2, '0')}: セリフ「${s.text}」 / 感情:${s.emotion} / 表情:${s.expression} / ポーズ:${s.pose} / 動き:${s.motion || '自然な小さな動き'} / 開始:${s.startPose || s.pose} / 終了:${s.endPose || s.pose} / ループ:${s.loop || '自然に最初へ戻る'} / フレーム:${s.frameCount || '8'}`).join('\n');
  return `あなたはLINEアニメーションスタンプ用のAPNG素材を制作するイラストレーターです。以下の設定で、${project.stamps.length}個のアニメーションスタンプを作成してください。\n\n【企画】${concept.concept}\n【利用シーン】${concept.usageScene}\n【セリフ方向性】${concept.dialogueDirection}\n\n【キャラクター】${character.name} / ${character.type}\n性格:${character.personality}\n外見:${character.appearance}\n服装:${character.clothing}\nメインカラー:${character.mainColor}\n画風:${character.artStyle}\n特徴:${character.features}\nNG:${character.negativePrompt}\n\n【APNG制作ルール】\n・各スタンプは背景完全透明のAPNG用フレームとして作成してください。\n・白背景、単色背景、風景背景、紙の質感、影付き背景は禁止です。\n・画像内に01〜08などの番号、管理ラベル、枠線、グリッド線を描かないでください。\n・描画してよい文字は各スタンプのセリフ本文だけです。\n・1つのアニメーションは5〜20フレームで、合計4秒以内に収まる自然な動きにしてください。\n・大きく移動させすぎず、LINEトーク上で見やすい小さくかわいい動きにしてください。\n・すべてのAPNGで同一キャラクターとして、顔、体型、配色、服装、線のタッチを統一してください。\n\n【作成するアニメーションスタンプ】\n${stampLines}\n\n各スタンプごとに、APNGとして書き出せる透過背景の連番フレーム、またはAPNGファイルを作成してください。`;
}
