...
return <><main className="mx-auto min-h-dvh max-w-md bg-zinc-50 pb-24"><AppHeader/><section className="px-4"><Link href="/projects/new" className="flex min-h-14 items-center justify-center rounded-2xl bg-emerald-500 px-4 font-bold text-white shadow-sm">＋ 新しいスタンプをつくる</Link><div className="mt-4 flex justify-end"><Link href="/settings" className="flex min-h-11 items-center rounded-xl border bg-white px-4 text-sm font-bold text-zinc-700">⚙︎ 設定・バックアップ</Link></div><h2 className="mb-3 mt-7 font-bold">あなたのプロジェクト</h2>{error&&<p className="mb-3 rounded-2xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}{loading?<p className="text-sm text-zinc-500">読み込み中...</p>:projects.length===0?<div className="rounded-3xl border border-dashed bg-white p-8 text-center text-sm text-zinc-500">まだプロジェクトがありません。<br/>最初のスタンプ制作を始めましょう。</div>:<div className="space-y-3">{projects.map(p=><ProjectCard key={p.id} project={p} onDuplicate={duplicate} onDelete={remove}/>)}</div>}</section></main><BottomNavigation/></>;}